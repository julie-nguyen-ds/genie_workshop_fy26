...
| 10–25 | **Ex 1 — Benchmarks (run + add your own).** Walk through `exercises/01_benchmarks/README.md`. Attendees open the Benchmark tab, confirm 6 pre-loaded questions, click Run benchmark. Don't tune anything; just look at each failure and call out which later exercise fixes it (use the table in the README). Then attendees add a 7th question of their own (verify the SQL in a SQL editor before adding). | This is the *baseline* — keep it brisk. The teaching arc lands at end-of-workshop when you re-run and watch the score climb. Coach them through the verification checklist (case, date column, join, sanity-check the number). |
...
| 37–62 | **Ex 3 — Fix oversized instructions.** Share the broken `space_id` you pre-seeded. Attendees open it, hit the wall, refactor. | This one frustrates people — that's the point. After 10 min if folks are stuck, do the refactor live and let them follow. |
